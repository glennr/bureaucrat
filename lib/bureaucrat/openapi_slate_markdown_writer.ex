defmodule Bureaucrat.OpenApiSlateMarkdownWriter do
  @moduledoc """
  This markdown writer integrates OAS 3.x (formerly Swagger) information and
  outputs in a slate-friendly markdown format.

  It requires that the decoded swagger data be available via
  `Application.get_env(:bureaucrat, :swagger)`, eg by passing it as an option to
  the `Bureaucrat.start/1` function.

  It can also be configured with the following options, set via the
  `:writer_opts` argument of Bureaucrat:

  * `:plaintext`: If `false`, will not render the plaintext version of
  request/response examples. Defaults to `true`
  """
  ...
  This corresponds to the `schemas` section of the OAS document.
  ...
  def write_models(file, swagger) do
    ...
    Enum.each(get_in(swagger, ~w(components schemas)), fn definition ->
